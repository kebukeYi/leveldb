...
class HandleTable {
 ...
 private:
  ...
  // 返回一个指向slot的指针，该指针指向与key/hash匹配的缓存条目。
  // 如果没有这样的缓存项，则返回一个指向相应链表末尾槽的指针。
...
class LRUCache {
 ...
 private:
  ...
  // Initialized before use. LRUCache的容量
  size_t capacity_;
  ...
  // Dummy head of in-use list. 缓存中存在,外部也有引用;
...
void LRUCache::Ref(LRUHandle* e) {
  if (e->refs == 1 && e->in_cache) {  // If on lru_ list, move to in_use_ list.
    LRU_Remove(e);// 冷链表进行删除;
    LRU_Append(&in_use_, e); // 链入到热链表;
  }
  e->refs++;
}

// 表示客户不再访问该元素，需要将引用计数－－，如果彻底没人用了，引用计数为0了，
...
Cache::Handle* LRUCache::Insert(const Slice& key, uint32_t hash, void* value,
  ...
  e->hash = hash;
  e->in_cache = false;
  e->refs = 1;  // for the returned handle. 引用+1, 返回调用者;
  std::memcpy(e->key_data, key.data(), key.size());

  // 容量 > 0 开启缓存模式;
  if (capacity_ > 0) {
    // 继续+1, 不仅会放在缓存中, 还会放在使用列表中;
